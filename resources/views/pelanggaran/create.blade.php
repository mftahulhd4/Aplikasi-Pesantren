            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 md:p-8 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('pelanggaran.store') }}" method="POST" class="space-y-8">
                        ...
                        {{-- BAGIAN 1: PILIH SANTRI (AJAX SEARCH) --}}
                        <div class="space-y-3">
                        ...
                        {{-- BAGIAN 2: DETAIL PELANGGARAN --}}
                        <div class="space-y-3">
                            <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200">2. Detail Pelanggaran</h3>
                            <div class="space-y-4">
                                <div>
                                    <x-input-label for="id_jenis_pelanggaran" :value="__('Jenis Pelanggaran (Wajib)')" />
                                    <select name="id_jenis_pelanggaran" id="id_jenis_pelanggaran" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 rounded-md shadow-sm" required>
                                        ...
                                        @foreach($jenisPelanggarans as $jenis)
                                            <option value="{{ $jenis->id_jenis_pelanggaran }}" {{ old('id_jenis_pelanggaran') == $jenis->id_jenis_pelanggaran ? 'selected' : '' }}>
                                                {{ $jenis->nama_pelanggaran }} (Minus: {{ $jenis->poin_minus }} Poin)
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    <x-input-label for="catatan_tindakan" :value="__('Catatan Hukuman / Takzir (Opsional)')" />
                                    <textarea id="catatan_tindakan" name="catatan_tindakan" rows="3" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 rounded-md shadow-sm" placeholder="Contoh: Membersihkan halaman masjid selama 3 hari.">{{ old('catatan_tindakan') }}</textarea>
                                    <x-input-error :messages="$errors->get('catatan_tindakan')" class="mt-2" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <x-primary-button class="ml-4 bg-red-600 hover:bg-red-700 focus:bg-red-700 focus:ring-red-500">{{ __('Simpan Catatan') }}</x-primary-button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#id_santri_select').select2({
                    ...
                    ajax: {
                        // Menggunakan rute pencarian khusus untuk pelanggaran
